...
(function(){
  ...
  theme.Variants = (function() {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function() {
        ...
        this._updateProductLocalTimeZone(this.variants,variant);
    ...
    _updateProductLocalTimeZone: function(){
  	const textSkip = ['We are looking forward to spending the summer together!'];
	var textHtmlSkip = document.getElementsByTagName('html')[0].innerHTML;
    var skipTimezone = false;


	var idVarianteActualTiempo = 0;
    var idVarianteActualFecha = 0;
    var variantesActualesTiempo = [];
    var variantesActualesFecha = [];
        
        
   	variantesActualesTiempo = Array.from(document.querySelectorAll("fieldset input[name='Time Options in Your Local Time']"), (fieldset) => fieldset);
	variantesActualesFecha = Array.from(document.querySelectorAll("fieldset input[name='Start Date']"), (fieldset) => fieldset);

      
    if(variantesActualesTiempo.length == 0 && variantesActualesFecha.length == 0){
      	variantesActualesTiempo = Array.from(document.querySelectorAll("fieldset input[name='本地时间中的时间选项']"), (fieldset) => fieldset);
		variantesActualesFecha = Array.from(document.querySelectorAll("fieldset input[name='开始日期']"), (fieldset) => fieldset);
    }

	for(var b=0; b< textSkip.length; b++) {if(textHtmlSkip.includes(textSkip[b])){skipTimezone = true;break}}
	
    if(variantesActualesTiempo.length > 0 && variantesActualesFecha.length > 0 && skipTimezone == false ){
		for(var b=0; b<variantesActualesTiempo.length; b++) {

          if(variantesActualesTiempo[b].checked){
              idVarianteActualTiempo = b;
          }

        }

        for(var b=0; b<variantesActualesFecha.length; b++) {

          if(variantesActualesFecha[b].checked){
              idVarianteActualFecha = b;
          }

        }
        //console.log(variantesActualesTiempo)
        //console.log(variantesActualesFecha)
        
        var currentDateVariant = (variantesActualesFecha[idVarianteActualFecha].defaultValue).trim();
        ...
        if(monthNames.get(currentMonthVariantArray[idMonth].toLowerCase()) || monthNames.get(currentMonthVariantArray[idMonth])){
          ...
          for(var b=0; b<variantesActualesTiempo.length; b++) {
            ...
            var nextDayValue = 0;
            var variantElements = "";
            //variantElements = document.getElementsByClassName("variant-input-wrap");
            ...
            currentTimeVariant = variantesActualesTiempo[b].defaultValue.replace('<transcy>', '').replace('</transcy>', '');
            ...
            if(currentTimeVariant){
              ...
              //labelVariante = document.getElementById(variantesActualesTiempo[b].id);
              labelVariante = Array.from(document.querySelectorAll("label[for='"+variantesActualesTiempo[b].id+"']"))
              if(labelVariante){
                ...
                if(localDate.getTimezoneOffset() <= 0){
                  labelVariante[0].textContent = localHour + ":"
  ...
  theme.CartForm = (function() {
    var selectors = {
      ...
      childNameInput: '#child-name',
      childAgeInput: '#child-age'
    ...
    function CartForm(form) {
      ...
      //Extra fields
      this.childNameInput = form.querySelector(selectors.childNameInput);
      this.childAgeInput = form.querySelector(selectors.childAgeInput);
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      onSubmit: function(evt) {
        ...
        //****Check extra fields
        if(this.childNameInput.value.length > 0 ){
			// continue to checkout
        }else{
        	alert(theme.strings.childNameRequired);
            this.submitBtn.classList.remove(classes.btnLoading)
            evt.preventDefault();
            return false;
        }
        
        if(this.childAgeInput.value.length > 0){
			// continue to checkout
        }else{
        	alert(theme.strings.childAgeRequired);
            this.submitBtn.classList.remove(classes.btnLoading)
            evt.preventDefault();
            return false;
        }
      ...
      cartMarkup: function(html) {
        ...
        if (count > 0) {
          ...
          /*Time Converter*/
          var listVariantsNodes = "";
          var posTextVarinatNodesTime = -1;
          var posTextVarinatNodesDate = -1;
          var searchTexts = ["Time Options in Your Local Time","Start Date","本地时间中的时间选项","开始日期"]
          var isEnglishSite = false;
        
    	  isEnglishSite = ( document.querySelector("#CartDrawerForm > div.drawer__fixed-header > div > div.h2.drawer__title").innerText == "大车" ? false : true)
          ...
          try {
            console.log(isEnglishSite," isEnglishSite")
            for(var zz = 0;zz < items.children.length;zz++){
              console.log(items.children[zz].children[1].children[0].children[1].children[0]);
              
              var idMonth = 0;
              var idMonthDay = 0;
              var currentMonthVariantArray = "";
              var valTextVariants =""
              
              if(isEnglishSite){
                valTextVariants = items.children[zz].children[1].children[0].children[1].children[0].childNodes[1].data.split(" ");
                idMonth = (parseInt(currentMonthVariantArray[1]) ? 2 : 1);
              	idMonthDay = (parseInt(currentMonthVariantArray[1]) ? 1 : 2);
                
              }else{
                valTextVariants = items.children[zz].children[1].children[0].children[1].children[0].childNodes[1].data;
                  ...
                  valTextVariants = items.children[zz].children[1].children[0].children[1].children[0].childNodes[2].innerHTML.split(" ");
                  idMonth = (parseInt(currentMonthVariantArray[1]) ? 1 : 0);
              	  idMonthDay = (parseInt(currentMonthVariantArray[1]) ? 0 : 1);
                
                }
              }
              currentMonthVariantArray = valTextVariants;
              console.log(valTextVariants);
              
              var currentMonthVariantNumber =0;
              var currentDayVariant=0;
              console.log(idMonth,idMonthDay,"idMonth idMonthDay");
              if(monthNames.get(currentMonthVariantArray[idMonth].toLowerCase()) || monthNames.get(currentMonthVariantArray[idMonth])){
              	currentMonthVariantNumber = monthNames.get(currentMonthVariantArray[idMonth].toLowerCase()) || monthNames.get(currentMonthVariantArray[idMonth]);
              	currentDayVariant = currentMonthVariantArray[idMonthDay];

                console.log(currentMonthVariantNumber,currentDayVariant);
                ...
                if(isEnglishSite){
                  valTextVariants = items.children[zz].children[1].children[0].children[1].children[1].childNodes[1].data
                  ...
                  if (valTextVariants != " ") {
                    ...
                    valTextVariants = items.children[zz].children[1].children[0].children[1].children[1].childNodes[2].innerHTML
                    

                  }
                }
                currentTimeVariant = valTextVariants;
                console.log(currentTimeVariant);
                ...
                if(localDate.getTimezoneOffset() <= 0){
                  ...
                  if(isEnglishSite){
                    items.children[zz].children[1].children[0].children[1].children[1].childNodes[1].data = timeChangeText;
                    ...
                    if (valTextVariants != " ") {
                      ...
                      items.children[zz].children[1].children[0].children[1].children[1].childNodes[2].innerHTML = timeChangeText;
  ...
  theme.headerNav = (function() {
    ...
    function stickyHeader() {
      ...
      window.on('scroll' + config.namespace, theme.utils.debounce(50, stickyHeaderHeight));
